...
-- Set cursor shape for Insert and Normal modes in a terminal
vim.opt.termguicolors = true
--vim.api.nvim_set_option('t_SI', '\e[6 q')  -- Cursor shape in Insert mode
-- Normal mode: Block cursor
--vim.api.nvim_set_option('t_EI', '\e[2 q')  -- Cursor shape in Normal mode
vim.api.nvim_create_autocmd('InsertEnter', {
  pattern = '*',
  command = 'set t_SI=▌', -- Block cursor in Insert mode
})

vim.api.nvim_create_autocmd('InsertLeave', {
  pattern = '*',
  command = 'set t_EI=•', -- Restore to default cursor when leaving Insert mode
})
...
local function toDoubleArr(obj)
  local obj_arr
...
local function keymap(modes, mappings)
  local modes_arr = toArr(modes)
  local mappings_arr = toDoubleArr(mappings)

  for i, mode in ipairs(modes_arr) do
    for j, mapping in ipairs(mappings_arr) do
...
function TmSearchNext()
  ...
  if cmd_type == '/' or cmd_type == '?' then
    -- If in search mode, go to next match and expand fold
...
local abbreviations = {
  ...
  vedt = 'tabf ~/.config/nvim/lua/custom/vimrc.lua',
  tedt = 'tabf ~/.termux/termux.properties',
  bedt = 'tabf ~/.bashrc',
  vrc = '~/.vim/vimrc',
  trc = '~/.termux/termux-properties',
  brc = '~/.bashrc',
  vsrc = 'source ~/.config/nvim/lua/custom/vimrc.lua',
}
